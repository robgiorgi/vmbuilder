...
class Firstscripts(Plugin):
    ...
    def post_install(self):
        ...
        if firstboot:
            ...
            self.context.install_file('/root/firstboot.sh', source=firstboot, mode=0o0700)
            os.rename('%s/etc/rc.local' % self.context.chroot_dir, '%s/etc/rc.local.orig' % self.context.chroot_dir)
            self.install_from_template('/etc/rc.local', 'firstbootrc', mode=0o0755)

        firstlogin = self.context.get_setting('firstlogin')
        if firstlogin:
            logging.debug("Installing first login script %s" % (firstlogin,))
            self.context.install_file('/root/firstlogin.sh', source=firstlogin, mode=0o0755)
        ...
        print("END of post_install (plugins/firstscripts/__init__.py)")
        return True
