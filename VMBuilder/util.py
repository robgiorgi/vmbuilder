...
# Import here to avoid having to build-dep at runtime...
from   Cheetah.Template import Template # requires: pip3 install Cheetah3
...
class NonBlockingFile(object):
    ...
    def set_non_blocking(self):
        flags = fcntl.fcntl(self.myfile, fcntl.F_GETFL)
        flags = flags | os.O_NONBLOCK
        fcntl.fcntl(self.myfile, fcntl.F_SETFL, flags)
    ...
    def process_input(self):
        data = self.myfile.read().decode("utf-8")
        if data == '':
            self.myfile.close()
            if self.logbuf:
...
def run_cmd(*argv, **kwargs):
    ...
    while not (mystdout.closed and mystderr.closed):
        ...
        fds = select.select([x.myfile for x in [mystdout, mystderr] if not x.closed], [], [])[0]
...
#    import Cheetah3
#    from   Cheetah3.Template import Template
    searchList = []
    if context:
...
def clean_up_tmpfs(mount_point):
    ...
    logging.info("UNMOUNTING (disk.py:clean_up_tmpfs)")
    ...
    try:
        ...
        logging.info('Unmounting tmpfs from %s UNSUCCESSFULL' % mount_point)
        pass
